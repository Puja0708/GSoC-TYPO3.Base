...
class TemplateCompiler {
	...
	public function get($identifier) {
		...
		if (!isset($this->syntaxTreeInstanceCache[$identifier])) {
			...
			$templateClassName = 'BaseCache_' . $identifier;
	...
	/**
	 * @param string $identifier
	 * @param \TYPO3\Base\Core\Parser\ParsingState $parsingState
	 * @return void
	 */
	public function store($identifier, \TYPO3\Base\Core\Parser\ParsingState $parsingState) {
		$identifier = $this->sanitizeIdentifier($identifier);
		...
		$classDefinition = 'class BaseCache_' . $identifier . ' extends \TYPO3\Base\Core\Compiler\AbstractCompiledTemplate';
...
public function getVariableContainer() {
	// TODO
	return new \TYPO3\Base\Core\ViewHelper\TemplateVariableContainer();
}
public function getLayoutName(\TYPO3\Base\Core\Rendering\RenderingContextInterface \$renderingContext) {
%s
return %s;
}
 ...
 * %s
 */
public function %s(\TYPO3\Base\Core\Rendering\RenderingContextInterface \$renderingContext) {
\$self = \$this;
%s
return %s;
}
...
EOD;
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode $node
	 * @return array two-element array, see above
	 * @throws \TYPO3\Base\Exception
	 */
	protected function convert(\TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode $node) {
		if ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\TextNode) {
			return $this->convertTextNode($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\NumericNode) {
			return $this->convertNumericNode($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\ViewHelperNode) {
			return $this->convertViewHelperNode($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode) {
			return $this->convertObjectAccessorNode($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\ArrayNode) {
			return $this->convertArrayNode($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\RootNode) {
			return $this->convertListOfSubNodes($node);
		} elseif ($node instanceof \TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode) {
			return $this->convertBooleanNode($node);
		} else {
			throw new \TYPO3\Base\Exception('Syntax tree node type "' . get_class($node) . '" is not supported.');
		}
	}

	/**
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\TextNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertTextNode(\TYPO3\Base\Core\Parser\SyntaxTree\TextNode $node) {
		return array(
			'initialization' => '',
	...
	/**
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\NumericNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertNumericNode(\TYPO3\Base\Core\Parser\SyntaxTree\NumericNode $node) {
		return array(
			'initialization' => '',
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\ViewHelperNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertViewHelperNode(\TYPO3\Base\Core\Parser\SyntaxTree\ViewHelperNode $node) {
		...
		if ($node->getUninitializedViewHelper() instanceof \TYPO3\Base\Core\ViewHelper\Facets\CompilableInterface) {
	...
	/**
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertObjectAccessorNode(\TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode $node) {
		return array(
			'initialization' => '',
			'execution' => sprintf('\TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode::getPropertyPath($renderingContext->getTemplateVariableContainer(), \'%s\', $renderingContext)', $node->getObjectPath())
		);
	}

	/**
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\ArrayNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertArrayNode(\TYPO3\Base\Core\Parser\SyntaxTree\ArrayNode $node) {
		...
		foreach ($node->getInternalArray() as $key => $value) {
			if ($value instanceof \TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode) {
	 ...
	 * @see convert()
	 */
	public function convertListOfSubNodes(\TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode $node) {
	...
	/**
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode $node
	 * @return array
	 * @see convert()
	 */
	protected function convertBooleanNode(\TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode $node) {
		...
		if ($node->getComparator() !== NULL) {
			...
			return array(
				...
				'execution' => sprintf('\TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode::evaluateComparator(\'%s\', %s, %s)', $node->getComparator(), $convertedLeftSide['execution'], $convertedRightSide['execution'])
			);
		} else {
			...
			return array(
				...
				'execution' => sprintf('\TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode::convertToBoolean(%s)', $converted['execution'])
			);
		}
	}
	 ...
	 * @return string
	 */
	public function wrapChildNodesInClosure(\TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode $node) {
		$closure = '';
