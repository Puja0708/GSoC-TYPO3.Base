...
class ViewHelperVariableContainer {
	...
	/**
	 *
	 * @var \TYPO3\Base\View\AbstractTemplateView
	 */
	protected $view;
	 ...
	 *
	 * @param string $viewHelperName The ViewHelper Class name (Fully qualified, like \TYPO3\Base\ViewHelpers\ForViewHelper)
	 * @param string $key Key of the data
	 * @param mixed $value The value to store
	 * @return void
	 * @throws \TYPO3\Base\Core\ViewHelper\Exception\InvalidVariableException if there was no key with the specified name
	 * @api
	 */
	public function add($viewHelperName, $key, $value) {
		if ($this->exists($viewHelperName, $key)) {
			throw new \TYPO3\Base\Core\ViewHelper\Exception\InvalidVariableException('The key "' . $viewHelperName . '->' . $key . '" was already stored and you cannot override it.', 1243352010);
		}
	...
	public function get($viewHelperName, $key) {
		if (!$this->exists($viewHelperName, $key)) {
			throw new \TYPO3\Base\Core\ViewHelper\Exception\InvalidVariableException('No value found for key "' . $viewHelperName . '->' . $key . '"', 1243325768);
		}
	...
	public function remove($viewHelperName, $key) {
		if (!$this->exists($viewHelperName, $key)) {
			throw new \TYPO3\Base\Core\ViewHelper\Exception\InvalidVariableException('No value found for key "' . $viewHelperName . '->' . $key . '", thus the key cannot be removed.', 1243352249);
		}
	 ...
	 *
	 * @param \TYPO3\Base\View\AbstractTemplateView $view View to set
	 * @return void
	 */
	public function setView(\TYPO3\Base\View\AbstractTemplateView $view) {
		$this->view = $view;
	}
	 ...
	 *
	 * @return \TYPO3\Base\View\AbstractTemplateView The View
	 */
