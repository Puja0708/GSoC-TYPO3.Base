<?php
namespace TYPO3\Base\Core\Parser;
...
class TemplateParser {
	...
	static public $SCAN_PATTERN_NAMESPACEDECLARATION = '/(?<!\\\\){namespace\s*(?P<identifier>[a-zA-Z]+[a-zA-Z0-9]*)\s*=\s*(?P<phpNamespace>(?:[A-Za-z0-9\.]+|Tx)(?:Base_NAMESPACE_SEPARATOR\w+)+)\s*}/m';
	...
	protected $namespaces = array(
		'f' => 'TYPO3\Base\ViewHelpers'
	);


	/**
	 * @var \TYPO3\Base\Core\Parser\Configuration
	 */
	...
	public function __construct() {
		self::$SCAN_PATTERN_NAMESPACEDECLARATION = str_replace('Base_NAMESPACE_SEPARATOR', preg_quote(\TYPO3\Base\Base::NAMESPACE_SEPARATOR), self::$SCAN_PATTERN_NAMESPACEDECLARATION);
	}

	/**
	 * Injects Base settings
	 *
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\Configuration $configuration
	 * @return void
	 */
	public function setConfiguration(\TYPO3\Base\Core\Parser\Configuration $configuration = NULL) {
	 ...
	 * @return \TYPO3\Base\Core\Parser\ParsedTemplateInterface Parsed template
	 * @throws Exception
	 */
	public function parse($templateString) {
		if (!is_string($templateString)) {
			throw new \TYPO3\Base\Core\Parser\Exception('Parse requires a template string as argument, ' . gettype($templateString) . ' given.', 1224237899);
		}
	 ...
	 * @throws \TYPO3\Base\Core\Parser\Exception if a namespace can't be resolved or has been declared already
	 */
	protected function extractNamespaceDefinitions($templateString) {
		...
		foreach ($matches as $match) {
			...
			if (array_key_exists($match['identifier'], $this->namespaces)) {
				throw new \TYPO3\Base\Core\Parser\Exception(sprintf('Namespace identifier "%s" is already registered. Do not re-declare namespaces!', $match['identifier']), 1331135889);
			}
		...
		foreach ($matches as $match) {
			if (array_key_exists($match['identifier'], $this->namespaces)) {
				throw new \TYPO3\Base\Core\Parser\Exception(sprintf('Namespace identifier "%s" is already registered. Do not re-declare namespaces!', $match['identifier']), 1224241246);
			}
	 ...
	 * @return \TYPO3\Base\Core\Parser\ParsingState
	 * @throws Exception
	 */
	protected function buildObjectTree($splitTemplate, $context) {
		...
		$state = $this->objectManager->get('TYPO3\Base\Core\Parser\ParsingState');
		...
		if ($state->countNodeStack() !== 1) {
			throw new \TYPO3\Base\Core\Parser\Exception('Not all tags were closed!', 1238169398);
		}
		return $state;
	}
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\ParsingState $state Current parsing state
	 ...
	 * @return void
	 */
	protected function openingViewHelperTagHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $namespaceIdentifier, $methodIdentifier, $arguments, $selfclosing) {
		...
		if ($selfclosing) {
			...
			$this->callInterceptor($node, \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_CLOSING_VIEWHELPER, $state);
		}
	}
	 ...
	 * @throws Exception
	 */
	protected function initializeViewHelperAndAddItToStack(\TYPO3\Base\Core\Parser\ParsingState $state, $namespaceIdentifier, $methodIdentifier, $argumentsObjectTree) {
		if (!array_key_exists($namespaceIdentifier, $this->namespaces)) {
			throw new \TYPO3\Base\Core\Parser\Exception('Namespace could not be resolved. This exception should never be thrown!', 1224254792);
		}
		...
		$currentViewHelperNode = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\ViewHelperNode', $viewHelper, $argumentsObjectTree);

		$state->getNodeFromStack()->addChildNode($currentViewHelperNode);

		if ($viewHelper instanceof \TYPO3\Base\Core\ViewHelper\Facets\ChildNodeAccessInterface && !($viewHelper instanceof \TYPO3\Base\Core\ViewHelper\Facets\CompilableInterface)) {
			$state->setCompilable(FALSE);
		}

			// PostParse Facet
		if ($viewHelper instanceof \TYPO3\Base\Core\ViewHelper\Facets\PostParseInterface) {
		...
		$this->callInterceptor($currentViewHelperNode, \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_OPENING_VIEWHELPER, $state);
	 ...
	 * before.
	 *
	 * @param array $expectedArguments Array of \TYPO3\Base\Core\ViewHelper\ArgumentDefinition of all expected arguments
	...
	protected function abortIfUnregisteredArgumentsExist($expectedArguments, $actualArguments) {
		...
		foreach (array_keys($actualArguments) as $argumentName) {
			if (!in_array($argumentName, $expectedArgumentNames)) {
				throw new \TYPO3\Base\Core\Parser\Exception('Argument "' . $argumentName . '" was not registered.', 1237823695);
			}
		}
	}
	...
	protected function abortIfRequiredArgumentsAreMissing($expectedArguments, $actualArguments) {
		...
		foreach ($expectedArguments as $expectedArgument) {
			if ($expectedArgument->isRequired() && !in_array($expectedArgument->getName(), $actualArgumentNames)) {
				throw new \TYPO3\Base\Core\Parser\Exception('Required argument "' . $expectedArgument->getName() . '" was not supplied.', 1237823699);
			}
		}
	}
	...
	protected function rewriteBooleanNodesInArgumentsObjectTree($argumentDefinitions, &$argumentsObjectTree) {
		foreach ($argumentDefinitions as $argumentName => $argumentDefinition) {
			if ($argumentDefinition->getType() === 'boolean' && isset($argumentsObjectTree[$argumentName])) {
				$argumentsObjectTree[$argumentName] = new \TYPO3\Base\Core\Parser\SyntaxTree\BooleanNode($argumentsObjectTree[$argumentName]);
			}
		}
	}
	...
	protected function resolveViewHelperName($namespaceIdentifier, $methodIdentifier) {
		...
		if (count($explodedViewHelperName) > 1) {
			$className = implode(\TYPO3\Base\Base::NAMESPACE_SEPARATOR, array_map('ucfirst', $explodedViewHelperName));
		} else {
			$className = ucfirst($explodedViewHelperName[0]);
		}
		$className .= 'ViewHelper';

		$name = $this->namespaces[$namespaceIdentifier] . \TYPO3\Base\Base::NAMESPACE_SEPARATOR . $className;
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\ParsingState $state The current parsing state
	 * @param string $namespaceIdentifier Namespace identifier for the closing tag.
	 * @param string $methodIdentifier Method identifier.
	 * @return void
	 * @throws \TYPO3\Base\Core\Parser\Exception
	 */
	protected function closingViewHelperTagHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $namespaceIdentifier, $methodIdentifier) {
		if (!array_key_exists($namespaceIdentifier, $this->namespaces)) {
			throw new \TYPO3\Base\Core\Parser\Exception('Namespace could not be resolved. This exception should never be thrown!', 1224256186);
		}
		$lastStackElement = $state->popNodeFromStack();
		if (!($lastStackElement instanceof \TYPO3\Base\Core\Parser\SyntaxTree\ViewHelperNode)) {
			throw new \TYPO3\Base\Core\Parser\Exception('You closed a templating tag which you never opened!', 1224485838);
		}
		if ($lastStackElement->getViewHelperClassName() != $this->resolveViewHelperName($namespaceIdentifier, $methodIdentifier)) {
			throw new \TYPO3\Base\Core\Parser\Exception('Templating tags not properly nested. Expected: ' . $lastStackElement->getViewHelperClassName() . '; Actual: ' . $this->resolveViewHelperName($namespaceIdentifier, $methodIdentifier), 1224485398);
		}
		$this->callInterceptor($lastStackElement, \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_CLOSING_VIEWHELPER, $state);
	}

	/**
	 * Handles the appearance of an object accessor (like {posts.author.email}).
	 * Creates a new instance of \TYPO3\Base\ObjectAccessorNode.
	 *
	 ...
	 * @return void
	 */
	protected function objectAccessorHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $objectAccessorString, $delimiter, $viewHelperString, $additionalViewHelpersString) {
		...
		if (strlen($objectAccessorString) > 0) {
			...
			$node = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode', $objectAccessorString);
			$this->callInterceptor($node, \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_OBJECTACCESSOR, $state);
	 ...
	 *
	 * @param \TYPO3\Base\Core\Parser\SyntaxTree\NodeInterface $node The syntax tree node which can be modified by the interceptors.
	 * @param integer $interceptionPoint the interception point. One of the \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_* constants.
	 * @param \TYPO3\Base\Core\Parser\ParsingState $state the parsing state
	 * @return void
	 */
	protected function callInterceptor(\TYPO3\Base\Core\Parser\SyntaxTree\NodeInterface &$node, $interceptionPoint, \TYPO3\Base\Core\Parser\ParsingState $state) {
	...
	protected function postProcessArgumentsForObjectAccessor(array $arguments) {
		foreach ($arguments as $argumentName => $argumentValue) {
			if (!($argumentValue instanceof \TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode)) {
				$arguments[$argumentName] = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\TextNode', (string)$argumentValue);
			}
		}
		return $arguments;
	}
	...
	protected function buildArgumentObjectTree($argumentString) {
		if (strpos($argumentString, '{') === FALSE && strpos($argumentString, '<') === FALSE) {
			return $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\TextNode', $argumentString);
		}
	 ...
	 * @return void
	 */
	protected function textAndShorthandSyntaxHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $text, $context) {
	 ...
	 * @return void
	 */
	protected function arrayHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $arrayText) {
		$state->getNodeFromStack()->addChildNode(
			$this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\ArrayNode', $this->recursiveArrayHandler($arrayText))
		);
	}
		...
		if (preg_match_all(self::$SPLIT_PATTERN_SHORTHANDSYNTAX_ARRAY_PARTS, $arrayText, $matches, PREG_SET_ORDER) > 0) {
			...
			foreach ($matches as $singleMatch) {
				...
				if (!empty($singleMatch['VariableIdentifier'])) {
					$arrayToBuild[$arrayKey] = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\ObjectAccessorNode', $singleMatch['VariableIdentifier']);
					...
					$arrayToBuild[$arrayKey] = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\ArrayNode', $this->recursiveArrayHandler($singleMatch['Subarray']));
				} else {
					throw new \TYPO3\Base\Core\Parser\Exception('This exception should never be thrown, as the array value has to be of some type (Value given: "' . var_export($singleMatch, TRUE) . '"). Please post your template to the bugtracker at forge.typo3.org.', 1225136013);
				}
			}
			return $arrayToBuild;
		} else {
			throw new \TYPO3\Base\Core\Parser\Exception('This exception should never be thrown, there is most likely some error in the regular expressions. Please post your template to the bugtracker at forge.typo3.org.', 1225136013);
		}
	}
	 ...
	 * Text node handler
	 *
	 * @param \TYPO3\Base\Core\Parser\ParsingState $state
	 * @param string $text
	 * @return void
	 */
	protected function textHandler(\TYPO3\Base\Core\Parser\ParsingState $state, $text) {
		$node = $this->objectManager->get('TYPO3\Base\Core\Parser\SyntaxTree\TextNode', $text);
		$this->callInterceptor($node, \TYPO3\Base\Core\Parser\InterceptorInterface::INTERCEPT_TEXT, $state);
