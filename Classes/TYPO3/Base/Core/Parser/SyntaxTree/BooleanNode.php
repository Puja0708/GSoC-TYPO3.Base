 ...
 */
class BooleanNode extends \TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode {
	 ...
	 */
	public function __construct(\TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode $syntaxTreeNode) {
		$childNodes = $syntaxTreeNode->getChildNodes();
		if (count($childNodes) > 3) {
			throw new \TYPO3\Base\Core\Parser\Exception('A boolean expression has more than tree parts.', 1244201848);
		...
		$this->leftSide = new \TYPO3\Base\Core\Parser\SyntaxTree\RootNode();
		$this->rightSide = new \TYPO3\Base\Core\Parser\SyntaxTree\RootNode();
		$this->comparator = NULL;
		foreach ($childNodes as $childNode) {
			if ($childNode instanceof \TYPO3\Base\Core\Parser\SyntaxTree\TextNode 
			...
			if ($this->comparator !== NULL) {
			...
			} elseif ($childNode instanceof \TYPO3\Base\Core\Parser\SyntaxTree\TextNode
				...
				if (isset($explodedString[0]) && trim($explodedString[0]) !== '') {
					...
					if (is_numeric($value)) {
						$this->leftSide->addChildNode(new \TYPO3\Base\Core\Parser\SyntaxTree\NumericNode($value));
					} else {
						$this->leftSide->addChildNode(new \TYPO3\Base\Core\Parser\SyntaxTree\TextNode(preg_replace('/(^[\'"]|[\'"]$)/', '', $value)));
					}
				}
				if (isset($explodedString[1]) && trim($explodedString[1]) !== '') {
					$value = trim($explodedString[1]);
					if (is_numeric($value)) {
						$this->rightSide->addChildNode(new \TYPO3\Base\Core\Parser\SyntaxTree\NumericNode($value));
					} else {
						$this->rightSide->addChildNode(new \TYPO3\Base\Core\Parser\SyntaxTree\TextNode(preg_replace('/(^[\'"]|[\'"]$)/', '', $value)));
					}
				}
			} else {
	...
	static public function evaluateComparator($comparator, $evaluatedLeftSide, $evaluatedRightSide) {
		switch ($comparator) {
			...
			case '<=':
				...
				return ($evaluatedLeftSide <= $evaluatedRightSide);
			default:
				throw new \TYPO3\Base\Core\Parser\Exception('Comparator "' . $comparator . '" is not implemented.', 1244234398);
		}
	}
