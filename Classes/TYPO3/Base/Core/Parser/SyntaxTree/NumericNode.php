 ...
 *
 */
class NumericNode extends \TYPO3\Base\Core\Parser\SyntaxTree\AbstractNode {
	...
	public function __construct($value) {
		if (!is_numeric($value)) {
			throw new \TYPO3\Base\Core\Parser\Exception('Numeric node requires an argument of type number, "' . gettype($value) . '" given.');
		}
		$this->value = $value + 0;
	}
	...
	public function addChildNode(\TYPO3\Base\Core\Parser\SyntaxTree\NodeInterface $childNode) {
		throw new \TYPO3\Base\Core\Parser\Exception('Numeric nodes may not contain child nodes, tried to add "' . get_class($childNode) . '".');
	}
}
