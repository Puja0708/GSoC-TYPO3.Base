...
class StandaloneView extends \TYPO3\Fluid\View\AbstractTemplateView {
	...
	/**
	 * @var \Packages\FluidStandAlone\\Mvc\ActionRequest
	 */
	protected $request;
	 ...
	 * Constructor
	 *
	 * @param \Packages\FluidStandAlone\\Mvc\ActionRequest $request The current action request. If none is specified it will be created from the environment.
	 */
	public function __construct(\Packages\FluidStandAlone\\Mvc\ActionRequest $request = NULL) {
	...
	public function initializeObject() {
		...
		$uriBuilder = new \Packages\FluidStandAlone\\Mvc\Routing\UriBuilder();
		$uriBuilder->setRequest($this->request);

		$this->setControllerContext(new \Packages\FluidStandAlone\\Mvc\Controller\ControllerContext(
			$this->request,
			new \Packages\FluidStandAlone\\Http\Response(),
			new \Packages\FluidStandAlone\\Mvc\Controller\Arguments(array()),
			$uriBuilder,
	 ...
	 *
	 * @return \Packages\FluidStandAlone\\Mvc\ActionRequest
	 */
	...
	protected function getLayoutPathAndFilename($layoutName = 'Default') {
		...
		$possibleLayoutPaths[] = \Packages\FluidStandAlone\\Utility\Files::getUnixStylePath($layoutRootPath . '/' . $layoutName . '.' . $this->getRequest()->getFormat());
		$possibleLayoutPaths[] = \Packages\FluidStandAlone\\Utility\Files::getUnixStylePath($layoutRootPath . '/' . $layoutName);
	...
	protected function getPartialPathAndFilename($partialName) {
		...
		$possiblePartialPaths[] = \Packages\FluidStandAlone\\Utility\Files::getUnixStylePath($partialRootPath . '/' . $partialName . '.' . $this->getRequest()->getFormat());
		$possiblePartialPaths[] = \Packages\FluidStandAlone\\Utility\Files::getUnixStylePath($partialRootPath . '/' . $partialName);
